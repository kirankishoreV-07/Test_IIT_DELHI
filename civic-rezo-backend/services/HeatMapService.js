...
class HeatMapService {
  constructor() {
    // Use environment variables or fallback to working values
  ...
  formatHeatMapData(complaints) {
    const heatMapPoints = complaints.map(complaint => {
      ...
      return {
        id: complaint.id,
        lat: parseFloat(complaint.location_latitude),
        lng: parseFloat(complaint.location_longitude),
  ...
  async insertMockComplaintData() {
    ...
    const mockComplaints = [
      // Coimbatore area complaints
      {
        ...
        priority_level: 'HIGH',
        priority_score: 0.85,
        location_description: 'Main Road, RS Puram',
        created_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), // 5 days ago
        resolved_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString()  // 2 days ago
      },
      {
        latitude: 11.0361540,
        longitude: 76.8928251,
        complaint_type: 'garbage_collection',
        status: 'pending',
        priority_level: 'CRITICAL',
        priority_score: 1.0,
        location_description: 'Navavoor Pirivu Area',
        created_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() // 1 day ago
      },
      {
        latitude: 11.0041,
        longitude: 76.9614,
        complaint_type: 'broken_streetlight',
        status: 'in_progress',
        priority_level: 'MEDIUM',
        priority_score: 0.65,
        location_description: 'Peelamedu Junction',
